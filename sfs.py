...
class fs:
    ...
    # DONE
        n = self.inodes[inum]
        if n.getRefCnt() == 1:
            self.dataFree(n.getAddr())
            self.inodeFree(inum)
        else:
            n.decRefCnt()
        parent = self.getParent(tfile)
        idx = self.nameToInum[parent]
        pn = self.inodes[idx]
        pn.decRefCnt()
        dataset = self.data[pn.getAddr()]
        dataset.delDirEntry(tfile)
    ...
    # DONE
        pn = self.inodes[self.nameToInum[parent]]
        pdata = pn.getAddr()
        if pdata.getFreeEntries() == 0:
            return -1
        if pdata.dirEntryExists(newfile):
            return -1
        n = self.nameToInum[target]
        pn.incRefCnt()
        pdata.addDirEntry(newfile, n)
        return n
